...
def displayUsage(rft, *argv, **kwargs):
    ...
    rft.printErr(
        "   {} [-VhvqS] -u<user> -p<passwd> -r<rhost[<port>] [-A <auth>] [-D <dir>]", prepend="  ")
...
def genTimeStatistics(mockupDir):
    ...
    for item in allResponseTimes:
        ...
        results['minResponseTime'] = (
            time, uri) if time < results['minResponseTime'][0] else results['minResponseTime']
        results['maxResponseTime'] = (
            time, uri) if time > results['maxResponseTime'][0] else results['maxResponseTime']

    results['averageResponseTime'] = round(
        results['totalResponseTime'] / len(allResponseTimes), 3)
    results['totalResponseTime'] = round(results['totalResponseTime'], 3)
    results['minResponseTime'] = (
        round(results['minResponseTime'][0], 3), results['minResponseTime'][1])
    results['maxResponseTime'] = (
        round(results['maxResponseTime'][0], 3), results['maxResponseTime'][1])
    return results
...
def main(argv):
    ...
    for opt, arg in opts:
        if opt in ("-h", "--help"):
            ...
            rft.auth = arg
            if rft.auth not in rft.authValidValues:
                rft.printErr("Invalid --Auth option: {}".format(rft.auth))
                rft.printErr("   Valid values: {}".format(
                    rft.authValidValues), noprog=True)
                sys.exit(1)
        else:
    ...
    if(rft.rhost is None):
        rft.printErr(
            "Error: -r rHost was not specified and is required by this command. aborting")
        displayUsage(rft)
        displayOptions(rft)
        sys.exit(1)
    if(rft.user is None):
        rft.printErr(
            "Error: -u user was not specified and is required. aborting")
        displayUsage(rft)
        displayOptions(rft)
        sys.exit(1)
    if(rft.password is None):
        rft.printErr(
            "Error: -p password was not specified and is required. aborting")
        displayUsage(rft)
        displayOptions(rft)
        sys.exit(1)
    ...
    rft.printVerbose(
        5, "Main: options parsed.  Now lookup subcommand and execute it")
    ...
    if (addCopyright is not None):
        if(type(d) is dict):
            ...
            rft.printErr("BUG: Expecting a dictionary for resource {} but got type: {}".format(
                absPath, type(d)))
    ...
    if(rc != 0):
        rft.printErr(
            "ERROR: Cant read root service:  GET /redfish/ from rhost. aborting")
    ...
    api = "odata"
    rc, r, j, d = rft.rftSendRecvRequest(
        rft.UNAUTHENTICATED_API, 'GET', rootUrl, relPath=api, jsonData=True)
    if(rc != 0):
        rft.printErr(
            "ERROR: Cant read mandatory API: /redfish/v1/odata. Continuing w/o creating mockup entry")
    else:
        dirPath = os.path.join(mockDir, "redfish", "v1", api)
        if(rfMakeDir(rft, dirPath) is False):
            rft.printErr(
                "ERROR: cant create directory: /redfish/v1/{}. aborting".format(api))
            sys.exit(1)
        filePath = os.path.join(dirPath, rfFile)
        with open(filePath, 'w', encoding='utf-8') as f:
            f.write(r.text)
    ...
    if(rc != 0):
        rft.printErr(
            "ERROR: Cant read mandatory API: /redfish/v1/$metadata. Continuing w/o creating mockup entry")
    else:
        dirPath = os.path.join(mockDir, "redfish", "v1", api)
        if(rfMakeDir(rft, dirPath) is False):
            rft.printErr(
        ...
        if scrapeMetadata:
            try:
                ...
                rft.printErr(
                    "ERROR: cant parse /redfish/v1/$metadata, cannot scrape the metadata for xml")
                root = []
            ...
            for ref in root:
                ...
                if not bool(urlparse(uri).netloc):
                    ...
                    if(rfMakeDir(rft, dirPath) is False):
                        rft.printErr(
                            "ERROR: cant create directory: {}. aborting".format(dirPath))
                        sys.exit(1)
                    rc, innerr, j, d = rft.rftSendRecvRequest(rft.UNAUTHENTICATED_API, 'GET', rootUrl, relPath=uri, jsonData=False,
                                                              headersInput=hdrs)
                    if(rc != 0):
                        rft.printErr(
                            "ERROR: Cant get {}, continuing w/o".format(uri))
                    else:
                        rft.printVerbose(
                            1, "Start Creating service xml: {}".format(uri))
    ...
    if(custom is True):
        rft.printVerbose(
            "Custom discovery of resources under root service specified")
        ...
        for rlink in rootLinks:
            ...
            if(rlink in rootRes):
                link = rootRes[rlink]
                rft.printVerbose(
                    1, "   Creating resource under root service navigation property: {}".format(rlink))
                rc, r, j, d = readResourceMkdirCreateIndxFile(
                    rft, rootUrl, mockDir, link, addCopyright, addHeaders, addTime)
                if(rc != 0):
                    rft.printErr(
                        "ERROR: got error reading root service resource--continuing. link: {}".format(link))
                resd = d
                ...
                if isCollection(resd) is True:  # (eg Systems, Chassis...)
                    for member in resd["Members"]:
                        rft.printVerbose(
                            4, "    Collection member: {}".format(member))
                        rc, r, j, d = readResourceMkdirCreateIndxFile(
                            rft, rootUrl, mockDir, member, addCopyright, addHeaders, addTime)
                        if(rc != 0):
                            rft.printErr(
                                "ERROR: got error reading root service collection member--continuing. link: {}".format(member))
                        memberd = d
                        sublinklist = resourceLinks[rlink]
                        rc, r, j, d = addSecondLevelResource(
                            rft, rootUrl, mockDir, sublinklist, memberd, addCopyright, addHeaders, addTime)
                        if(rc != 0):
                            rft.printErr(
                                "ERROR: Error processing 2nd level resource (8)--continuing. link:{}".format(member))
                # its not a collection. (eg accountService) do the 2nd level resources now
                else:
                    sublinklist = resourceLinks[rlink]
                    rc, r, j, d = addSecondLevelResource(
                        rft, rootUrl, mockDir, sublinklist, resd, addCopyright, addHeaders, addTime)
                    if(rc != 0):
                        rft.printErr(
        ...
        processed = set()
        recursive_call(rft, rootv1data, rootUrl, mockDir, processed,
...
def get_nav_and_collection_properties(rft, rs, exceptionList):
    ...
    if (any(x in rs['@odata.id'] for x in exceptionList)):
        ...
        for k, v in get_items(rs):
            rft.printVerbose(
                2, '   Checking for nav properties in item with k = {}, v = {}'.format(k, v))
                    ...
                    if isinstance(x, dict):
                        ...
                        # handle case of location uri references to JSON schemas and messages registries
                            ...
                            if odata_id is not None:
                                rft.printVerbose(
                                    2, '   Appending {} to nav_list'.format(odata_id))
    ...
    if not nav_list:
        ...
        # Returns a list of navigation properties, if they are present.
        return (nav_list)
...
def rfMakeDir(rft, dirPath):
    ...
    try:
        ...
        if(ee.errno == errno.EEXIST):
            ...
            rft.printErr(
                "ERROR: rfMakeDir: Error creating directory: {}".format(ee.errno))
            success = False
    return(success)
...
def readResourceMkdirCreateIndxFile(rft, rootUrl, mockDir, link, addCopyright, addHeaders, addTime, jsonData=True):
    ...
    if "@odata.id" not in link:
        rft.printErr(
            "ERROR:readResourceMkdirCreateIndxFile: no @odata.id property in link: {}".format(link))
    ...
    if(rfMakeDir(rft, dirPath) is False):
        rft.printErr("ERROR:readResourceMkdirCreateIndxFile: for link:{}, path:{}, cant create directory: {}. aborting".format(
            link, absPath, dirPath))
        return(5, r, False, None)
    ...
    if os.path.isfile(filePath) is True:
        rft.printErr(
            "ERROR: index.json file already exists in this directory {} --continuing".format(filePath))
...
def isCollection(resource):
    if "Members" in resource:
        # its a collection if it has a Members array  (sort of cheating)
        return True
    else:
        return False
